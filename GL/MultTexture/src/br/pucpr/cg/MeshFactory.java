...
public class MeshFactory {
    ...
    public static Mesh loadTerrain(File file, float scale, int texRepeat) throws IOException {
        ...
        for (int z = 0; z < depth; z++) {
            for (int x = 0; x < width; x++) {
                ...
                int tone4 = x < width-1? new Color(img.getRGB(x+1, z)).getRed() : tone1;
                int tone5 = z < depth-1 ? new Color(img.getRGB(x, z+1)).getRed() : tone1;*/
    ...
    private static List<Vector2f> createBlendMap(List<Vector4f> texWeights, int width, int height)
    {
    		...
    		for (int j = 0; j < height; j++) 
    		{
    			 ...
    			 for(int ky = 0; ky < 3; ky++)
                 {
                     for(int kx = 0; kx < 3; kx++)
                     {
                         ...
                         r2 = (int) (pixel.getRed() * kernel[kx][ky]);
                         g2 = (int) (pixel.getGreen() * kernel[kx][ky]);
                         b2 = (int) (pixel.getBlue() * kernel[kx][ky]);
                         a2 = (int) (pixel.getAlpha() * kernel[kx][ky]);
    	...
    	try {
			ImageIO.write(img, "png", new File("img/opengl/textures/BlendMap.png"));
		} catch (IOException e) {
